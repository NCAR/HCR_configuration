...
#===========================================================================
#
# Produce plots for GEOREF differences for aristo
#
#===========================================================================
...
def main():
    ...
    parser.add_option('--file',
                      dest='compFilePath',
                      default='../data/aristo/ARISTO.rf04.20170302_154000.to.20170302_013000.txt',
...
def loadDataArrays(compData, compTimes):
    ...
    global temp, tempSdn500, tempTailcone
    ...
    tempSdn500 = np.array(compData["tempSec"]).astype(np.double)
...
def doPlotOverview():
    ...
    ax2.plot(ctimes, tempSdn500, label='Sdn500 temp (C)', color='blue', linewidth=1)
...
def doPlotPitchRoll():
    ...
    ax1.plot(ctimes, pitchCmSm, label='PitchSdn500', color='red', linewidth=1)
    ...
    ax2.plot(ctimes, rollCmSm, label='RollSdn500', color='red', linewidth=1)
    ...
    ax3.plot(ctimes, pitchDiffSm, \
             label='pitchDiffSdn500', color='green', linewidth=1)
    ...
    ax4.plot(ctimes, rollDiffSm, \
             label='rollDiffSdn500', color='blue', linewidth=1)
...
#             label='altDiff', color='red', linewidth=2)
    ...
    fig.suptitle("DIFFS GV minus SDN500 - file " + os.path.basename(options.compFilePath))
...
def doPlotEstPitchDiff():
    ...
    fig.suptitle("ESTIMATED PITCH DIFF: GV-SDN500 - file " + os.path.basename(options.compFilePath))
