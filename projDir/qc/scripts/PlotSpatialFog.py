...
#===========================================================================
#
# Produce plots for SPATIAL FOG data
#
#===========================================================================
...
def loadDataArrays(compData, compTimes):
    ...
    global bodyAccelX, bodyAccelY, bodyAccelZ
    bodyAccelX = movingAverage(np.array(compData["bodyAccelX_m_s-2"]).astype(np.double), filtLen)
    bodyAccelY = movingAverage(np.array(compData["bodyAccelY_m_s-2"]).astype(np.double), filtLen)
    bodyAccelZ = movingAverage(np.array(compData["bodyAccelZ_m_s-2"]).astype(np.double), filtLen)

    global bodyRotX, bodyRotY, bodyRotZ
    bodyRotX = movingAverage(np.array(compData["bodyAngVelX_deg_s-1"]).astype(np.double), filtLen)
    bodyRotY = movingAverage(np.array(compData["bodyAngVelY_deg_s-1"]).astype(np.double), filtLen)
    bodyRotZ = movingAverage(np.array(compData["bodyAngVelZ_deg_s-1"]).astype(np.double), filtLen)

    global sensorAccelX, sensorAccelY, sensorAccelZ
    sensorAccelX = movingAverage(np.array(compData["sensorAccelX_m_s-2"]).astype(np.double), filtLen)
    sensorAccelY = movingAverage(np.array(compData["sensorAccelY_m_s-2"]).astype(np.double), filtLen)
    sensorAccelZ = movingAverage(np.array(compData["sensorAccelZ_m_s-2"]).astype(np.double), filtLen)

    global sensorRotX, sensorRotY, sensorRotZ
    sensorRotX = movingAverage(np.array(compData["sensorAngVelX_deg_s-1"]).astype(np.double), filtLen)
    sensorRotY = movingAverage(np.array(compData["sensorAngVelY_deg_s-1"]).astype(np.double), filtLen)
    sensorRotZ = movingAverage(np.array(compData["sensorAngVelZ_deg_s-1"]).astype(np.double), filtLen)
...
def doPlotBodyAccel():
    ...
    ax1.plot(ctimes, bodyAccelX, label='BodyAccelX(m/s2)', color='green', linewidth=1)
    ax2.plot(ctimes, bodyAccelY, label='BodyAccelY(m/s2)', color='red', linewidth=1)
    ax3.plot(ctimes, bodyAccelZ, label='BodyAccelZ(m/s2)', color='blue', linewidth=1)

    configTimeAxis(ax1, -9999, -9999, "BodyAccelX", 'upper right')
    configTimeAxis(ax2, -9999, -9999, "BodyAccelY", 'upper right')
    configTimeAxis(ax3, -9999, -9999, "BodyAccelZ", 'upper right')
    ...
    fig.suptitle("FOG BODY ACCELERATION - " + str(startTime) + " to " + str(endTime))
...
def doPlotBodyRot():
    ...
    ax1.plot(ctimes, bodyRotX, label='BodyRotX(deg/s)', color='green', linewidth=1)
    ax2.plot(ctimes, bodyRotY, label='BodyRotY(deg/s)', color='red', linewidth=1)
    ax3.plot(ctimes, bodyRotZ, label='BodyRotZ(deg/s)', color='blue', linewidth=1)

    configTimeAxis(ax1, -9999, -9999, "BodyRotX", 'upper right')
    configTimeAxis(ax2, -9999, -9999, "BodyRotY", 'upper right')
    configTimeAxis(ax3, -9999, -9999, "BodyRotZ", 'upper right')
    ...
    fig.suptitle("FOG BODY ROTATION - " + str(startTime) + " to " + str(endTime))

    return

########################################################################
# Plot sensor accelerations

def doPlotSensorAccel():
    ...
    ax1.plot(ctimes, sensorAccelX, label='SensorAccelX(m/s2)', color='green', linewidth=1)
    ax2.plot(ctimes, sensorAccelY, label='SensorAccelY(m/s2)', color='red', linewidth=1)
    ax3.plot(ctimes, sensorAccelZ, label='SensorAccelZ(m/s2)', color='blue', linewidth=1)

    configTimeAxis(ax1, -9999, -9999, "SensorAccelX", 'upper right')
    configTimeAxis(ax2, -9999, -9999, "SensorAccelY", 'upper right')
    configTimeAxis(ax3, -9999, -9999, "SensorAccelZ", 'upper right')
    ...
    fig.suptitle("FOG SENSOR ACCELERATION - " + str(startTime) + " to " + str(endTime))

    return

########################################################################
# Plot sensor rotations
...
def doPlotSensorRot():
    ...
    ax1.plot(ctimes, sensorRotX, label='SensorRotX(deg/s)', color='green', linewidth=1)
    ax2.plot(ctimes, sensorRotY, label='SensorRotY(deg/s)', color='red', linewidth=1)
    ax3.plot(ctimes, sensorRotZ, label='SensorRotZ(deg/s)', color='blue', linewidth=1)

    configTimeAxis(ax1, -9999, -9999, "SensorRotX", 'upper right')
    configTimeAxis(ax2, -9999, -9999, "SensorRotY", 'upper right')
    configTimeAxis(ax3, -9999, -9999, "SensorRotZ", 'upper right')
    ...
    fig.suptitle("FOG SENSOR ROTATION - " + str(startTime) + " to " + str(endTime))
