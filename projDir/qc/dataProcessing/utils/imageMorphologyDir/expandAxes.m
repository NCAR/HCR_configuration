...
%    figure
%    h(1)=axes('pos',[0.13 0.11 0.33 0.81]);
%    imshow('cameraman.tif');
%    r1 = drawrectangle(h(1),'Position',[82 22 84 67]);
%    r2 = drawrectangle(h(1),'Position',[173 96 49 76]);
%    h(2)=axes('pos',[0.57 0.11 0.33 0.81]);
%    imshow('peppers.png');
%    expandAxes(h)
%
% MOTIVATION:
...
% 03/19/2019
%    Modified to support images.roi objects (e.g., 'drawrectangle'), since
%    these objects don't have buttondown functions. 
...
for ii = 1:numel(hndls)
    ...
    if strcmp(get(hndls(ii),'type'),'axes')
        ...
        % allchildren(cellfun(@(x)strcmp(x,'text'),get(allchildren,'type'))) = [];
        % All children WITH EMPTY BUTTONDOWNFCNs
        for jj = 1:numel(allchildren)
            if isempty(get(allchildren(jj),'buttondownfcn'))
                try
                    set(allchildren(jj),'buttondownfcn',{@expandIt,hndlSet,rotEnable});
                end
            end
        end
    ...
    function expandIt(varargin)
        ...
        switch selType
            ...
            case 'normal'
                ...
                newfigChildren = findobj(new_fig);
                for jj = 1:numel(newfigChildren)
                    try
                        set(newfigChildren(jj),'buttondownfcn',@figureClicked);
                    end
                end
