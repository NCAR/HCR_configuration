...
function [BBfinishedOut iceLevAsl offset]= f_meltLayer_advanced(data,zeroAdjustMeters,thresholds)
...
[layerAlts,layerInds,layerAltsAdj,layerIndsAdj,tempDataY]=zeroDegIso(data,zeroAdjustMeters,oneGate,oddAngles);


%% Remove data that is not suitable
VELdata=data.VEL_MASKED(:,tempDataY);

LDRdata=data.LDR(:,tempDataY);
LDRdata(isnan(VELdata))=nan;
...
%% Tightened backlobe
% Initiate mask
blMask=zeros(size(VELdata));

tempDBZ=data.DBZ_MASKED(:,tempDataY);
...
BBfinishedOrigInds=nan(size(data.DBZ_MASKED));
