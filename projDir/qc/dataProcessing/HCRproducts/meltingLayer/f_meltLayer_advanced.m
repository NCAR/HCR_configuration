...
%% Truncate to non missing and regions with sub 10 deg temps
gapSecs=10;
nonMissingInds=findNonMissingInds(data,gapSecs);

% Temperature too high
minTemp=min(data.TEMP,[],1,'omitnan');
nonMissingInds(minTemp>10)=0;
...
data.VEL_MASKED(data.VEL_MASKED<0)=nan;
velDiff=diff(data.VEL_MASKED,1);
...
ylimits=[0,5];
    ...
    ax4=subplot(4,1,4);
    hold on;
    surf(newTime,newASL(1:end-1,:)./1000,newVELdiff,'edgecolor','none');
    view(2);
    ax4.Colormap=jet;
    caxis([0 0.6]);
    colorbar
    ...
    print([figdir,'meltLayer',datestr(data.time(1),formatOut),'_to_',datestr(data.time(end),formatOut)],'-dpng','-r0');
