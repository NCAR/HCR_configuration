function[class, m]=precip_cld(dBZ,LDR, vel, sigmav,temp);
vel=abs(vel);
%   Membership functions for particle detection

% 1:Beta  2:Delta  

  w=[40 15 15 10 20]; %w=[30 20 20 10 20];  
  %w=[50 30 20];
...
%  Membership functions for Reflectivity
  ...
  %vm(7,1,:,:)=trapmf(dBZ,[0,5,15,20]); % Snow
  m(7,1,:,:)=trapmf(dBZ,[10,12,18,20]); % Snow
  %m(7,1,:,:)=trapmf(dBZ,[0,5,10,15]); % Snow JV
...
%  Membership functions for velocity
   ...
   %vm(7,4,:,:)=zmf(sigmav,[0.4, 0.3]); % Snow
   m(7,4,:,:)=smf(sigmav,[0.4, 0.5]); % Snow
%    m(8,4,:,:)=smf(sigmav,[0.4, 0.5]); % Mixed phase
  ...
  %  Membership functions for Temperature
   ...
   %m(3,5,:,:)=smf(temp,[272., 275.]); % Drizzle
...
result(8,inoldr)=0;
   ...
   zerosMat(find(resMinusMax==0))=1;
      
   class=nan(size(dBZ));
   
   for ii=1:8
       testMat=squeeze(zerosMat(ii,:,:));
       class(find(isnan(class) & testMat==1))=ii;
   end
