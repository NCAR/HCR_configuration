...
removeStart=datetime(removeTimesIn(:,1),removeTimesIn(:,2),removeTimesIn(:,3),...
    removeTimesIn(:,4),removeTimesIn(:,5),0);
removeEnd=datetime(removeTimesIn(:,6),removeTimesIn(:,7),removeTimesIn(:,8),...
    removeTimesIn(:,9),removeTimesIn(:,10),0);
...
for aa=1:14
    ...
    while startTime<endTime1
        ...
        %% Remove times that were used for tuning

        removeInds=[];
        for jj=1:length(removeStart)
            ri=find(data.time>=removeStart(jj) & data.time<=removeEnd(jj));
            removeInds=cat(2,removeInds,ri);
        end

        if isfield(data,'PID')
            data.PID(:,removeInds)=nan;
        end

        if isfield(data,'HCR_PID')
            data.HCR_PID(:,removeInds)=nan;
        end
...
centers2={0.125:0.25:0.875 0.125:0.25:0.875};
...
N2=hist3(liqFracUW_HCR,'Ctrs',centers2);
N2(N2==0)=nan;
N2norm=N2'./size(liqFracUW_HCR,1).*100;

N2norm(isnan(N2norm))=0;
