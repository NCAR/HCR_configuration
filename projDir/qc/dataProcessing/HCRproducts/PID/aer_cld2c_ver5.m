function[class]=aer_cld2c(Beta, Delta,temp);
...
%  Membership functions for Depolarization ratio
   ...
   %vm(7,2,:,:)=trapmf(Delta,[[0.0,0.01,0.04, 0.05]]);  %v Adhoc for drizzle; need to be revised
...
%  Membership functions for Temperature
   ...
   %vm(6,3)=trapmf(temp,[246.,248.,300.,302.]);
   m(2,3,:,:)=trapmf(temp,[271.,273.,300.,305.]);
   %vm(7,3)=smf(temp,[280., 283.]);
   %m(7,3,:,:)=smf(temp,[274., 300.]);
...
for ii=1:7
   ...
   % class=find((result-max(result)==0),1);
   %if isnan(Beta) | isnan(Delta) 
    %  class(1)=1.;
  %end
   %class=find(max(result));
  %class=max(result)
