function[class, m]=precip_aerosol(Beta, Delta,dBZ,LDR, vel, sigmav,temp);
vel=abs(vel);
...
%  Membership functions for velocity
   ...
   %m(3,3,:,:)=trapmf(vel,[.5,1.5,3,4]); % Drizzle
  ...
  %m(3,1,:,:)=trapmf(Beta,[0.8e-6,1.2e-6,1.8e-5,2e-5]);% Aerosol 2
...
%  Membership functions for Depolarization ratio
   ...
   %m(3,2,:,:)= trapmf(Delta,[-0.1,0.01,0.25,0.30]);
