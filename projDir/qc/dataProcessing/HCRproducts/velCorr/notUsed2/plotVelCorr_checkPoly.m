...
testCases=readtable(['/h/eol/romatsch/hcrCalib/velCorr/',project,'/testCasesCheck.dat']);

figdir=['/h/eol/romatsch/hcrCalib/velCorr/',project,'/velFigsCheck/'];
...
% %         data.VEL_RAW(:,posInds2)=data.VEL_RAW(:,posInds2)-2.*data.nyquist_velocity(posInds2);
        ...
        velAngCorr=data.VEL_RAW+xxCorr+yyCorr+zzCorr;
...
%         velCorrOut(:,data.elevation>0)=velAngCorr(:,data.elevation>0);
        ...
        plot(data.time,data.VEL(linInd),'-b','linewidth',1.5);
        plot(data.time,velAngCorr(linInd),'-c','linewidth',1.5);
        ylim([nanmedian(data.VEL(linInd))-1.5 nanmedian(data.VEL(linInd))+1.5]);
        ylabel('Velocity [m/s]');
        legend('VelSurfRaw','VelSurf','VelSurfCalc','Orientation','horizontal');
        ...
        if savefig
            ...
            print(f1, [figdir,'line_',datestr(startTime,formatOut),'_to_',datestr(endTime,formatOut)],'-dpng','-r0');
        end
