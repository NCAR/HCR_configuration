...
for aa=1:size(loopInds,1)
    ...
    if ismember(ii,plotRangeInds) & ~isempty(plotTime)
        ...
        if ~plotAll
            ...
            print(f1,[figdir,'spectra/',project,'/spectra_',datestr(plotTime,'yyyymmdd_HHMMSS_'),num2str(ii),'.png'],'-dpng','-r0');
        end
    end
end
...
if ~isempty(plotTime) & ~all(isnan(velOut(:)))
    ...
    if ~plotAll
        set(gcf,'PaperPositionMode','auto')
        print(f1,[figdir,'waterfall/',project,'/waterfall_',datestr(plotTime,'yyyymmdd_HHMMSS'),'.png'],'-dpng','-r0');
    end
end
end