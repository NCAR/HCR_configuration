function [sigWidthCorrRMnoise,noiseThresh,peaksOut]=noiseFloorRM_HilSekRaw(sigWidthCorr,rawPow,noiseStd,sigPeaks,sigValleys,testVel,fakeMeanVel,aa)
...
sampleNum=size(rawPow,2);
avNum=3;
rawPowBig=repmat(rawPow,1,3);
rawPowMov=movmean(rawPowBig,avNum);
rawPowMov=rawPowMov(sampleNum+1:2*sampleNum);

[noiseThresh,meanNoise,R2]=findNoiseThresh(rawPowMov,avNum);
