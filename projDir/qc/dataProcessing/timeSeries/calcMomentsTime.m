function momentsTime=calcMomentsTime(cIQv,cIQh,prt,ii,momentsTime,data)
...
momentsTime.vel(:,ii)=data.lambda/(4*pi*prt)*angle(R1v);
momentsTime.width(:,ii)=data.lambda/(2*pi*prt*6^.5)*abs(log(abs(R1v./R2v))).^0.5;
