	...
	real freq,sfcpres,height(n),temp(n),humidity(n),
     .  vap(n),cloudlwc(n),extinct(n),totExt
	open(unit=3,file='atmosnTest1.dat',status='unknown')
 ...
 10     continue
	...
	do 20 i=1,num_layers
           totExt=totExt+(extinct(i)*4.343*(height(i)-height(i+1)))
           write(*,*) 'Total Extinction', totExt
...
110     CONTINUE
        ...
        write(6,*)l,'height1',height(l),'height2',height(L+1)
	write(6,*)l,'temp',T(NUMZ),'pressure',PRES,'RH',RH(NUMZ)
	write(6,*)l,'extinction',extinct(l)*4.343
120   CONTINUE
...
C****         NOTE:  SLIGHTMODIFICATION OF MODEL ON 8/31/82.
