...
directories.planeDir=['/scr/snow2/rsfdata/projects/socrates/GV/'];
...
for ii=1:size(caseList,1)
    ...
    planeAlt2in=ncread(planeFileIn,'ALT');
    planeAlt2=planeAlt2in(1,:)';
    
    try
        testALT_A=ncread(planeFileIn,'ALT_A');
        if exist('testALT_A')
            disp('ALT_A found.');
        end
    end
    ...
    try
        testALT_A2=ncread(planeFileIn,'ALT_A2');
        if exist('testALT_A2')
            disp('ALT_A2 found.');
        end
    end
    ...
    TThcr=timetable(PLT.time,PLT.alt);
    TTplane=timetable(planeTime,planeAlt,planeAlt2);
    ...
    hold on
    %plot(altFromRange);
    plot(PLT.time,PLT.alt,'-c','linewidth',1.5);
    plot(PLT.time,PLT.range,'-g','linewidth',1.5);
    plot(TTcombined.Time,TTcombined.planeAlt,'-b','linewidth',1.5);
    ylabel('Meters');
    ...
    plot(TTcombined.Time,TTcombined.planeAlt-TTcombined.Var1,'-k','linewidth',1.5);
    %plot(TTcombined.Time,TTcombined.planeAlt2-TTcombined.Var1,'-b','linewidth',1.5);
    %legend('HCR altitude','Range to surf','Range-HCR alt','GGALT-HCR alt','ALT-HCR alt');
    legend('HCR altitude','Range to surf','GGALT','Range-HCR alt','GGALT-HCR alt');
    ylabel('Meters');
        
    title(['RF ',num2str(ii),', altitude of ocean surface'],'interpreter','none');
