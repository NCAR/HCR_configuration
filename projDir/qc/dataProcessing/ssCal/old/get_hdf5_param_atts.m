function [ ATT GATT ] = get_hdf5_param_atts( f_name )
%
...
natts = size( myinfo.Attributes, 1);

for jj=1:natts;
   t_attname = myinfo.Attributes(jj).Name;
   t_attname = regexprep( deblank(t_attname ), '^_', '');
   GATT.(t_attname) = myinfo.Attributes(jj).Value;
end;
