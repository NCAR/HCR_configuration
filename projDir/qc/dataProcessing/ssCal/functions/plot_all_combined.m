function plot_all_combined(PLTall,sigXlims,sigYlims,reflXlims,reflYlims,projName,directories,uniqueCases)
...
for ii=1:size(PLTall,1)
    legendGet{end+1}=uniqueCases{ii}.timest{1};
...
for ii=1:size(PLTall,1)
    if min(isnan(smSig0measured(:,ii)))==0
        legendGet2{end+1}=uniqueCases{ii}.timest{1};
