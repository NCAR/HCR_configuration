...
function [alphaTot,gammaTot,wdspd,wddir]= f_atten_layers_sfcWnd(soundFile,f,varargin)
...
ptrhalt=cat(2,soundData(:,5),soundData(:,6),soundData(:,8),soundData(:,14),soundData(:,11),soundData(:,12));

%Remove whole rows if there is a missing value in any of the sounding
%variables
ptrhalt(any(ptrhalt(:,1:4)==-999,2),:) = [];
...
[sfcWndRow sfcWndCol]=find(ptrhalt(:,5)>-900 & ptrhalt(:,6)>-900);
if ptrhalt(max(sfcWndRow),4)<20;
    wdspd=ptrhalt(max(sfcWndRow),5);
    wddir=ptrhalt(max(sfcWndRow),6);
else
    wdspd=nan;
    wddir=nan;
end
