...
f2 = figure('DefaultAxesFontSize',11);
...
if isfield(PLT,'sig0model') & max(~isnan(PLT.sig0model))>0
    ...
    if ~isnan(SST)
        text(xlims(2)-10,ylims(2)-1.5,['SST: ',num2str(SST,2),' C'],'fontsize',12);
    end
end
if ~isnan(wdSpeed)
    text(xlims(2)-10,ylims(2)-3,['Surf. wind speed: ',num2str(wdSpeed,2),' m s^{-1}'],'fontsize',12);
end
if ~isnan(atten)
    text(xlims(2)-10,ylims(2)-4.5,['Two way atten: ',num2str(atten*2,2),' dB'],'fontsize',12);
end
if ~isnan(windDir)
    text(xlims(2)-10,ylims(1)+3,['Wind direction: ',num2str(windDir,2),' deg'],'fontsize',12);
end
if ~isnan(hdgMinus180)
    text(xlims(2)-10,ylims(1)+1.5,['Heading-180: ',num2str(hdgMinus180,2),' deg'],'fontsize',12);
end
set(gca,'XLim',xlims);
set(gca,'YLim',ylims);
...
if isfield(PLT,'sig0model') & max(~isnan(PLT.sig0model))>0
    ...
    legend([l1,l2,l3,l4,l5,l7],{'Rot angle <= 180 deg','Rot angle > 180 deg','FV model','Wu model','CM model','Data fit'},'location','southwest','fontsize',12);
else
