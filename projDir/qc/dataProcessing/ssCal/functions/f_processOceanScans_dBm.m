...
function [outTable PLTall]= f_processOceanScans_dBm(directories,infile,makeFigs,projName)
...
for ii=1:size(uniqueCases,1); %Loop through all cases
    ...
    if ~isnan(uniqueCases{ii,1}.soundg{1})
        ...
        elseif soundFile==2
            ...
            oneCaseFileList=makeFileList(directories.dataDir,startTime,endTime,1);
...
waves=0;
intable=uniqueCases{1,:};
if any(strcmp('wave', intable.Properties.VariableNames)) & isfield(directories,'wavedir')
    waves=1;
    attenuation.sigWaveHeight=nan(size(uniqueCases,1),1);
    attenuation.swellWaveHeight=nan(size(uniqueCases,1),1);
    attenuation.swellWaveDir=nan(size(uniqueCases,1),1);
    
    for ii=1:size(uniqueCases,1); %Loop through all cases
        
        if strcmp(uniqueCases{ii,1}.wave{1},'nan')
            uniqueCases{ii,1}.wave{1}=str2num(uniqueCases{ii,1}.wave{1});
        end
        
        if ~isnan(uniqueCases{ii,1}.wave{1})
            %get lat lon alt
            % make list with files to go through
            startTimeStr=(uniqueCases{ii,1}.timest{:});
            endTimeStr=(uniqueCases{ii,1}.timend{:});
            ...
            %model
            waveFile=[directories.wavedir uniqueCases{ii,1}.wave{1}];
            waveLat=ncread([waveFile 'sig_wav_ht.nc'],'lat_240');
            waveLon=ncread([waveFile 'sig_wav_ht.nc'],'lon_240');
            latIndW=find(abs(waveLat-round(latS(1)))<0.005);
            lonIndW=find(abs(waveLon-round(lonS(1)))<0.005);
            
            sigWaveHeight=ncread([waveFile 'sig_wav_ht.nc'],'HTSGW_240_SFC');
            attenuation.sigWaveHeight(ii)=sigWaveHeight(lonIndW,latIndW);
            swlWaveHeight=ncread([waveFile 'swl_wav_ht.nc'],'SWELL_240_SFC');
            attenuation.swellWaveHeight(ii)=swlWaveHeight(lonIndW,latIndW);
            swlWaveDir=ncread([waveFile 'swl_wav_dir.nc'],'SWDIR_240_SFC');
            attenuation.swellWaveDir(ii)=swlWaveDir(lonIndW,latIndW);
        end
    end
end
...
for ii=1:size(uniqueCases,1); %Loop through all cases
    ...
    % calculate bias
    [PLTall{ii},bias.liebe(ii),bias.itu(ii),bias.stdLiebe(ii),bias.N(ii)]=f_determine_bias_dBm(PLTall{ii},attenuation.liebe(ii),attenuation.itu(ii),frq);
    ...
    PLTall{ii}=f_sigma0_model(PLTall{ii},attenuation.windspeed(ii));
end

%% Create table
if waves
    outTable=cell2table(cell(size(uniqueCases,1)+2,15), ...
        'VariableNames', {'StartDate','EndDate','BiasLiebe','BiasITU','Std','N','OneWayAttLiebe','OneWayAttITU', ...
        'MeanHdg','Sounding','SfcWindSpd','SfcWindDir','SigWaveHt','SwellWaveHt','SwellWaveDir'});
else
...
if waves
    outTable.SigWaveHt=[nan;nan;attenuation.sigWaveHeight];
    outTable.SwellWaveHt=[nan;nan;attenuation.swellWaveHeight];
    outTable.SwellWaveDir=[nan;nan;attenuation.swellWaveDir];
end
