...
attenuationFrom='file'; %Which attenuation data should be used: ecmwf, era5
...
for ii=1:size(caseList,1)
    ...
    while startTime<endTimeAll
        ...
        if ~isempty(data.dbz)
            ...
            if isfield(directories,'modeldir')
                modeldata.topo=[];
            ...
            if ~max(data.reflMask)==0
                %% Attenuation
                if strcmp(attenuationFrom,'file')
                    [attenuation.liebe{end+1},attenuation.itu{end+1},attenuation.windspeed{end+1},attenuation.u{end+1},attenuation.v{end+1},attenuation.sst{end+1}]= ...
                        f_atten_layers_file(frq/1e+9,data.time,1,data);
                else
                    [attenuation.liebe{end+1},attenuation.itu{end+1},attenuation.windspeed{end+1},attenuation.u{end+1},attenuation.v{end+1},attenuation.sst{end+1}]= ...
