...
for c=1:ToTest %for each sample
    [~,~,dist]=kmeans(X,c,'MaxIter',1000); %compute the sum of intra-cluster distances
    tmp=sum(dist); %best so far
    
    for cc=2:Repeats %repeat the algo
        [~,~,dist]=kmeans(X,c,'MaxIter',1000);
...
[IDX,C,SUMD]=kmeans(X,K,'MaxIter',1000); %now rerun one last time with the optimal number of clusters
%C=C.*(MAX-MIN)+MIN;
end