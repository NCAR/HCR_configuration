...
figdir=[dataDirCF(1:end-5),'microPhys/kmeansMo/'];
outdir=[dataDirCF(1:end-5),'microPhys/matFiles/'];
...
for aa=1:length(caseStart)
    ...
    specData=dataCF;
    specData=rmfield(specData,'VEL_RAW');
    specData=rmfield(specData,'VEL_CORR');
    specData=rmfield(specData,'VEL_MASKED');
    specData=rmfield(specData,'eastward_velocity');
    specData=rmfield(specData,'northward_velocity');
    
    if isfield(specData,'LDR_MASKED')
        specData.LDR=specData.LDR_MASKED;
        specData=rmfield(specData,'LDR_MASKED');
    end

    specData.VEL=momentsSpecSmoothCorr.velRaw;
    specData.WIDTH=momentsSpecSmoothCorr.width;
    specData.SKEW=momentsSpecSmoothCorr.skew;
    specData.KURT=momentsSpecSmoothCorr.kurt;
    specData.EE_WIDTH=momentsSpecSmoothCorr.lrwidth;
    specData.L_SLOPE=momentsSpecSmoothCorr.lslope;
    specData.R_SLOPE=momentsSpecSmoothCorr.rslope;
    specData.LE_VEL=momentsSpecSmoothCorr.level;
    specData.RE_VEL=momentsSpecSmoothCorr.revel;
    specData.LP_VEL=momentsSpecSmoothCorr.lpvel;
    specData.RP_VEL=momentsSpecSmoothCorr.rpvel;

    specVars=fields(specData);
    for kk=1:length(specVars)
        if size(specData.(specVars{kk}),1)>1
            specData.(specVars{kk})(isnan(dataCF.VEL_MASKED))=nan;
        end
    end

    save([outdir,project,'_',datestr(specData.time(1),'yyyymmdd_HHMMSS'),'_to_',datestr(specData.time(end),'yyyymmdd_HHMMSS'),'.mat'],'specData');
end
