...
figdir=[dataDirCF(1:end-5),'microPhys/'];
...
for aa=1:length(caseStart)
    ...
    micLabels=kmeansLabels(momentsTime,momentsSpecSmoothCorr);
    ...
    f1 = figure('Position',[200 500 700 400],'DefaultAxesFontSize',12,'visible',showPlot);
    ...
    hold on
    surf(momentsSpecSmoothCorr.time,momentsSpecSmoothCorr.asl./1000,micLabels,'edgecolor','none');
    view(2);
    ylabel('Altitude (km)');
    clim([0.5,10.5]);
    ...
    ylim(ylims);
    xlim([momentsSpecSmoothCorr.time(1),momentsSpecSmoothCorr.time(end)]);

    set(gcf,'PaperPositionMode','auto')
    print(f1,[figdir,project,'_kmLabels_',datestr(momentsSpecSmoothCorr.time(1),'yyyymmdd_HHMMSS'),'_to_',datestr(momentsSpecSmoothCorr.time(end),'yyyymmdd_HHMMSS')],'-dpng','-r0');
