function [powerRMnoiseAvRM,velOut,peakVelsOut,peakPowsOut]=noisePeaksAirVel_noWidthCorr(specDB,velIn,data,firFilt,filtShift,widthC,plotTime,figdir)
...
for aa=1:size(loopInds2,1)
    ...
    corrAW=aircraftWidthCorr_noWidthCorr(powerRMnoiseRawPlot(ii,:),widthC,velOut(ii,:),noiseThreshAll(ii),powerRMnoiseAv(ii,:),plotTime,plotRangeInds,ii,figdir,showPlot);
end
