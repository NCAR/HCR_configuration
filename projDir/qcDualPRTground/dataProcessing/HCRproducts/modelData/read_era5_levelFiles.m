function era5data = read_era5_levelFiles(indir,startTime,endTime,SSTyes)
...
for kk=1:length(inhours) %Loop through all hours
    ...
    uFiles=dir([indir,'U.*',dayStr,'00_',dayStr,'23.nc']);
    vFiles=dir([indir,'V.*',dayStr,'00_',dayStr,'23.nc']);
    ...
    for jj=1:size(tFiles,1)
        ...
        u(:,:,jj)=fliplr(squeeze(ncread([uFiles(jj).folder,'/',uFiles(jj).name],'U',[1,1,1,timeInd],[inf,inf,inf,1])));
        v(:,:,jj)=fliplr(squeeze(ncread([vFiles(jj).folder,'/',vFiles(jj).name],'V',[1,1,1,timeInd],[inf,inf,inf,1])));
    end
