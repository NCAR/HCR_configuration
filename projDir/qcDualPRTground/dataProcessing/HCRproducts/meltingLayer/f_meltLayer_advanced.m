...
velDiff=nan(size(dataShort.VEL));
...
% Median filter to smoth data
dataShort.DBZ=medfilt2(dataShort.DBZ,[3,7]);

% Erode in the vertical to get rid of edges
dbzMask=~isnan(dataShort.DBZ);
dbzMask=imerode(dbzMask,strel('line',15,90));
dbzEroded=dataShort.DBZ;
...
dbzDiff=nan(size(dataShort.DBZ));
...
linprob=sub2ind(size(dataShort.VEL),probMaxGetInd,colprob);
...
%% Melting layer mask
medFilledMat=repmat(medFilled,size(dataShort.DBZ,1),1);
...
data.velDiff=nan(size(data.DBZ));
data.velDiff(:,nonMissingInds==1)=velDiff;

data.dbzDiff=nan(size(data.DBZ));
data.dbzDiff(:,nonMissingInds==1)=dbzDiff;

data.meltMask=zeros(size(data.DBZ));
data.meltMask(:,nonMissingInds==1)=meltMask;

data.meltProb=nan(size(data.DBZ));
...
maxMat=repmat(maxAltSall,size(data.DBZ,1),1);
