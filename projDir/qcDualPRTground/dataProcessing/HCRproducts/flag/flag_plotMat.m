...
figdir=[dataDir(1:end-14),'flagPlots/wholeIOPs/'];
...
for aa=1:size(caseList,1)
    ...
    fileIn1=dir([modeldir,whichModel,'.flagfield',ls,'.*.IOP',num2str(aa),'.mat']);
    modIn=load([modeldir,fileIn1.name]);    
    flag=modIn.flagField;
       
    fileIn1=dir([modeldir,whichModel,'.antstat',ls,'.*.IOP',num2str(aa),'.mat']);
    modIn=load([modeldir,fileIn1.name]);    
    antstat=modIn.antennaStat;
        
    fileIn1=dir([modeldir,whichModel,'.time',ls,'.*.IOP',num2str(aa),'.mat']);
    modIn=load([modeldir,fileIn1.name]);    
    ...
    while startPlot<endTime
        ...
        print(f1,[figdir,project,'_IOP',num2str(aa),'_flag_',ls,'_',datestr(timePlot(1),'yyyymmdd_HHMMSS'),'_to_',datestr(timePlot(end),'yyyymmdd_HHMMSS')],'-dpng','-r0')
